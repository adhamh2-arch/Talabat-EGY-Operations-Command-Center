/**
 * TrackerWriter.gs — v5.4 (32-column live schema + Chain ID at AF + Action Plan persistence).
 * Writes daily offenders into the Vendor Action Tracker (cols A–AE / 1–31) AND stamps the
 * Chain ID into AF (col 32) at daily-write time, sourced straight from the feeder data
 * (DataReader captures chainId -> ScoringEngine.topOffenders.chainId). This removes the
 * reliance on ActionTool.refreshChainIds() as the only way AF gets filled, so the Tuesday
 * portal action agent always has a real Chain ID on freshly-written rows.
 ...
 *
 * v5.4 (July 2026): AC (Action Plan Shared) now defaults to 'No' instead of blank on every
 * new row — ActionForm.gs (the AM-facing web form) flips it to 'Yes' on submit, so the
 * column always reads an explicit Yes/No rather than blank. Also: before today's rows are
 * deleted + re-inserted (the dedup-refresh below), any Action Plan already logged for TODAY
 * is captured by Vendor ID and carried forward into the fresh rows — otherwise a same-day
 * re-run of dailyRun would silently wipe an AM's submitted plan back to No/blank.
 */
...
function updateVendorTracker(scoredOutput, feedData, emailsSent) {
  ...
  // Capture any Action Plan an AM already submitted TODAY (via the ActionForm.gs web app)
  // before we delete + re-insert today's block below, keyed by Vendor ID. Without this, a
  // same-day re-run would reset AC/AD back to 'No'/blank and lose the submission.
  var todaysPlans = {};
  for (var p = 1; p < existing.length; p++) {
    if (rowDateStr_(existing[p][0]) !== today) continue;
    var pvid = String(existing[p][1] || '').trim();
    if (!pvid) continue;
    todaysPlans[pvid] = { ac: existing[p][28], ad: existing[p][29] }; // AC / AD (0-indexed)
  }
  ...
  scoredOutput.clusterResults.forEach(function(cluster) {
    (cluster.topOffenders || []).forEach(function(v) {
      ...
      var acVal = (prior && prior.ac) ? prior.ac : 'No'; // defaults to 'No' until the AM submits the Action Plan form
      var adVal = (prior && prior.ad) ? prior.ad : '';   // carries forward today's submitted plan across re-runs
      rows.push([
        today,
        v.vendorId || '',
        v.vendor,
