...
function getNeedActionQueue() {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    // Only act on food vendors that are NOT Key Accounts (is_key=No, is_food=Yes)
    var isKey  = String(r[TCOL.isKeyVip - 1] || '').trim().toLowerCase();
    var isFood = String(r[TCOL.isFood  - 1] || '').trim().toLowerCase();
    if (isKey === 'yes') continue;   // skip Key Accounts
    if (isFood !== 'yes') continue;  // skip non-food vendors
...
function logPortalAction(vendorId, foundStatus, setStatus) {
  ...
  // Tracker rows are written NEWEST-FIRST (today's block sits at the top, row 2 down).
  // Scan top-down so we update TODAY's row for this vendor, not its oldest historical
  // occurrence — scanning bottom-up (the old behavior) matched the oldest row instead,
  // which meant Action Taken never landed on the row sendActionedVendorEmails() reads.
  for (var i = 0; i < ids.length; i++) {
