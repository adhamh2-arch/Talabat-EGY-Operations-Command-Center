...
// Web App URL for the per-vendor Action Plan form (ActionForm.gs, added July 2026).
// Deployment created 2026-07-21 (Execute as: Me, Access: talabat.com domain).
...
// Root-cause options shown on the Action Plan form (ActionForm.gs) — kept here so the
// form and any future reporting can share one source of truth.
var ROOT_CAUSE_OPTIONS = [
  'Vendor Closed / Not Accepting Orders',
  'Rejected Orders (Busy / Overloaded)',
  'Wrong Order Prepared',
  'Missing Items',
  'Food Quality Issue',
  'Late Delivery / Prep Time',
  'Staffing Issue',
  'Other'
];

// Builds the per-vendor Action Plan form link embedded in the daily AM email
// (EmailBuilder2.gs actionsTable_). Falls back to the plain tracker link if the web app
// hasn't been deployed yet (ACTION_FORM_URL blank).
...
// ===== TEST MODE SWITCH (added July 2026) =====
...
// (Main.gs: enableTestMode / disableTestMode / showEmailMode).
// Defaults to ON (safe) until you explicitly GO LIVE, so triggers can be installed
// and smoke-tested without ever emailing real Team Leaders / AMs.
function isTestMode() {
