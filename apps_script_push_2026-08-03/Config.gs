...
var ACTION_FORM_URL = 'https://script.google.com/a/macros/talabat.com/s/AKfycby3tXMiwtH6MIdPu19IFDbuLIr4GwIEaATXIUj_FbosM-8Ax6UZh_oTx5BNN8PX-dqF/exec';
var ROOT_CAUSE_OPTIONS = ['Vendor Closed / Not Accepting Orders','Rejected Orders (Busy / Overloaded)','Wrong Order Prepared','Missing Items','Food Quality Issue','Late Delivery / Prep Time','Staffing Issue','Other'];
function buildActionFormUrl_(vendorId, runDate) {
  if (!ACTION_FORM_URL) return TRACKER_URL;
  return ACTION_FORM_URL + '?vid=' + encodeURIComponent(vendorId) + '&rd=' + encodeURIComponent(runDate);
}
var FEEDERS = {
...
Object.keys(CLUSTERS).forEach(function(k) { CLUSTERS[k].key = k; CLUSTERS[k].targetLabel = (CLUSTERS[k].target * 100).toFixed(2) + '%'; });
var SEGMENT_TARGETS = { TGO: { label: 'TGO', target: 0.0065, targetLabel: '0.65%' }, TMP: { label: 'TMP', target: 0.0075, targetLabel: '0.75%' } };
var FEED_COLS = { vendorId: 0, vendor: 1, city: 2, amName: 3, amEmail: 4, teamLeaderName: 5, isTgo: 6, isKey: 7, isFood: 8, zone: 9, failReason: 10, chainName: 11, chainId: 12, orderDate: 13, netOrders: 14, failedOrders: 15, lostGmv: 16, lateDeliveryCases: 17, missingItemCases: 18, qualityCases: 19, partialRefund: 20, contactRate: 21 };
var FEED_HEADER_KEY = 'dim_vendor_info_vendor_id';
var FEED_TAB_PREFERENCE = ['Extract 1', 'Connected Sheet 1'];
var SHEET_NAMES = { executiveSummary: 'Executive Summary', operationalPerformance: 'Operational Performance', financialImpact: 'Financial Impact', rootCauseAnalysis: 'Root Cause Analysis', riskMonitoring: 'Risk Register', vendorTracker: 'Vendor Action Tracker', qualityReview: 'Quality Review', monitoring: 'Monitoring Log', auditTrail: 'Audit Trail', projectStructure: 'Project Structure' };
var THEME = { primary: '#FF5900', primaryDark: '#B34700', primaryTint: '#FCA06F', primaryBg: '#FFF3E7', cream: '#F4EDE3', burgundy: '#411517', lime: '#CFFF00', critical: '#D32F2F', high: '#FF5900', medium: '#FFB300', low: '#2E7D32', headerText: '#FFFFFF', muted: '#8A7F76' };
...
function getCairoDate(offsetDays) { var d = new Date(); if (offsetDays) d.setDate(d.getDate() + offsetDays); return Utilities.formatDate(d, 'Africa/Cairo', 'yyyy-MM-dd'); }
function getCairoMonth() { return Utilities.formatDate(new Date(), 'Africa/Cairo', 'MMMM yyyy'); }
function getMtdRange() {
  ...
  return { start: start, end: end, days: dayNum, label: Utilities.formatDate(new Date(now.getFullYear(), now.getMonth(), 1), 'Africa/Cairo', 'MMM d') + ' - ' + Utilities.formatDate(now, 'Africa/Cairo', 'd, yyyy') };
}
function normKey_(s) { return String(s == null ? '' : s).trim().replace(/\.+$/, '').toLowerCase(); }
...
function buildDashboardUrl_(clusterKey) {
  ...
  if (f.isKey) { p['Is Key VIP Account (Yes / No)'] = 'Yes'; p['Is Food (Yes / No)'] = 'Yes'; }
  else { p['Is Food (Yes / No)'] = 'Yes'; p['Is Key VIP Account (Yes / No)'] = 'No'; if (f.tl) p['Team Leader Name'] = f.tl.join(','); if (f.cities) { p['Vendor City'] = f.cities.join(','); if (c.tlName) p['Team Leader Name'] = c.tlName; } }
  var qs = Object.keys(p).map(function(k) { return lookerEnc_(k) + '=' + lookerEnc_(p[k]); }).join('&');
  return LOOKER_DASHBOARD + '?' + qs;
}
function lookerEnc_(s) { return encodeURIComponent(String(s)).replace(/%20/g, '+').replace(/%2C/g, ','); }
function isTestMode() { var v = PropertiesService.getScriptProperties().getProperty('TEST_MODE'); return v === null ? true : (v === 'true'); }
function setTestMode_(on) { PropertiesService.getScriptProperties().setProperty('TEST_MODE', on ? 'true' : 'false'); }
